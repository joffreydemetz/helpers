...
{
	/**
	 * @var    mixed    Option to perform case-sensitive sorts.  Boolean or array of booleans.
	 */
	protected static $sortCase;

	/**
	 * @var    mixed    Option to set the sort direction. Integer or array of integers.
	 */
	protected static $sortDirection;
  
	/**
	 * Option to set the object key to sort on.
	 * @var    string   Option to set the object key to sort on.
	 */
	protected static $sortKey;

	/**
	 * @var    mixed    Option to perform a language aware sort. Boolean or array of booleans.
	 */
	protected static $sortLocale;
  
	/**
	 * Multidimensional array_merge_recursive
	 * @return 	array
  ...
  public static function array_merge_recursive()
  {
      ...
      foreach($array as $key => $value){
        if ( is_string($key) ){
          if ( is_array($value) && array_key_exists($key, $merged) && is_array($merged[$key]) ){
            $merged[$key] = call_user_func([__NAMESPACE__ . '\\Arrayhelper', __METHOD__], $merged[$key], $value);
          }
          else {
            $merged[$key] = $value;
          }
	...
	/**
	 * Utility function to map an array to a integer value
	 * @param 	array     &$array     The array to map.
	 * @param 	int|null  $default    Default value
	 * @return 	array   The mapped array mapped
	 */
	public static function toInteger(&$array, $default=0)
  {
		if ( is_array($array) ){
	...
	public static function toObject(&$array, $class='\\stdClass')
	{
		...
		if ( is_array($array) ){
			...
			foreach($array as $k => $v){
				if ( is_array($v) ){
					$obj->$k = self::toObject($v, '\\stdClass');
				}
				else {
					$obj->$k = $v;
				}
